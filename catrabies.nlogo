turtles-own
  [ sick?                ;; if true, the turtle is infectious
    remaining-immunity   ;; how many weeks of vaccination the cat has left
    sick-time            ;; how long, in weeks, the cat has been infectious
    age ]                ;; how many weeks old the cat is

globals
  [ %infected            ;; what % of the population is infectious
    %vaccinated          ;; what % of the population is vaccinated
    lifespan             ;; the lifespan of a cat
    chance-reproduce     ;; the probability of a cat generating an offspring each tick
    carrying-capacity    ;; the number of cat that can be in the world at one time
    vaccination-duration ]  ;; how many weeks vaccination lasts
...
;; We create a variable number of cats of which 10 are infectious,
;; and distribute them randomly
to setup-turtles
  create-turtles Number-of-cats
...
to get-sick ;; cat procedure
  set sick? true
  set remaining-immunity 0
end

to get-healthy ;; cat procedure
  set sick? false
  set remaining-immunity 0
  set sick-time 0
end

to become-immune ;; cat procedure
  set sick? false
  set sick-time 0
  set remaining-immunity vaccination-duration
end

;; This sets up basic constants of the model.
to setup-constants
  set lifespan 13 * 52      ;; 13 times 52 weeks = 13 years = 677 weeks old
  set carrying-capacity 300
  set chance-reproduce 3
  set vaccination-duration 52
end
...
to update-display
  ask turtles
    [ if shape != turtle-shape [ set shape turtle-shape ]
      set color ifelse-value sick? [ red ] [ ifelse-value vaccinated? [ grey ] [ green ] ] ]
end
...
to get-older ;; turtle procedure
  ...
  set age age + 1
  if age > lifespan [ die ]
  if vaccinated? [ set remaining-immunity remaining-immunity - 1 ]
...
to infect ;; turtle procedure
  ask other turtles-here with [ not sick? and not vaccinated? ]
